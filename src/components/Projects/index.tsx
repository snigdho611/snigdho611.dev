...
const Projects = () => {
  return (
    <motion.div
      ...
      className="w-3/4 mx-auto my-[600px] sm:my-72 h-72 flex flex-wrap gap-5"
    >
      ...
      {data.map(({ id, title, description, stacks, url }) => {
        return (
          <motion.a
            href={url}
            whileHover={{ y: -15 }}
            className="flex flex-col gap-3 grow-0 px-4 py-8 shrink basis-full mx-auto lg:basis-[30%] border-2 border-cyan-400 rounded-lg"
            key={id}
          >
